...
public class JigsawPuzzle : MonoBehaviour {
    ...
    public Vector2 size = new Vector2(4,2);     // how many pieces will this puzzle have (x,y)
                    ...
                    {										
						...
						if (Input.touchCount==0 || (Input.touchCount>0 && dragTouch.fingerId == Input.touches[0].fingerId))
						{
	                        ...
	                        else
	                        {
	                            ...
	                            RaycastHit[] hits = Physics.RaycastAll(Camera.main.ScreenPointToRay(position),Vector3.Distance(Camera.main.transform.position, transform.position)*100000000, 1 << main.layerMask);
                                Debug.Log(hits.Length + "..." + Vector3.Distance(Camera.main.transform.position, transform.position));
                                Debug.DrawRay(Camera.main.ScreenPointToRay(position).origin, Camera.main.ScreenPointToRay(position).direction, Color.red, Vector3.Distance(Camera.main.transform.position, transform.position) * 200);
	                            if (hits.Length>0)
	                            {
									// we have hit something so check them
	                                for (int h=0; h<hits.Length; h++)
	                                {
                                        Debug.Log(hits[h].collider.gameObject);
